...
class StoryList {
  ...
  async addStory( user, newStory ) {
    ...
    const response = await axios.post("https://hack-or-snooze-v3.herokuapp.com/stories", { "token": user.loginToken,
    "story": {
  ...
  {
       console.log(id);
        let story = this.stories.filter((x)=> {
          return x.storyId == id;
       })
       return story[0];
  }
...
class User {
  ...
  //adds or removes the story from user's favorites list. If favorites page is being displayed, removes the unfavorited story from page
  async  toggleFavorite(story)
  {
    const token = this.loginToken;
     if (checkifStoryFavorited(story.storyId) === 2)
    {
      await axios({
      ...
      {
         if (this.favorites[x].storyId === story.storyId)
         {
    ...
    else if (checkifStoryFavorited(story.storyId) === 1)
    {
